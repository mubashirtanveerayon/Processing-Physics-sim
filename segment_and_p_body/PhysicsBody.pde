...
class PhysicsBody {
  ...
  void collideWithSegment(Segment w) {
    PVector collision = w.getVectorFromClosestPoint(position);
    float collisionMag = collision.mag();
    if (collisionMag>radius) return;
    PVector collisionNormal = collision.normalize();
    float penetration = radius - collisionMag;
    collisionNormal.mult(penetration + w.reactionStrength);
   //position.sub(collisionNormal.normalize().copy().mult(penetration));
   applyLinearImpulse(collisionNormal);
